    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col gap-4 justify-between h-full">
          {rating > 0 && (
            <div className="flex gap-1" role="img" aria-label={`Rating: ${Math.min(Math.max(rating, 0), 5)} out of 5 stars`}>
              {Array.from({ length: 5 }).map((_, index) => (
                <Star
                  key={index}
                  size={16}
                  className={cn(
                    index < Math.min(Math.max(rating, 0), 5)
            ...
            <div className="flex items-center gap-4">
              {image && (
                <Avatar className="h-12 w-12">
                  <AvatarImage src={image} alt={name} className="h-full w-full object-cover" />
