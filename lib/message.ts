...
export class MessageEngine {
  ...
  private _onAudioMetadata: ((metadata: Uint8Array) => void) | null = null;
  private _onStreamMessage: ((uid: UID, payload: Uint8Array) => void) | null = null;
  ...
  private _listenRtcEvents() {
    ...
    this._onAudioMetadata = (metadata: Uint8Array) => {
      const pts64 = Number(new DataView(metadata.buffer).getBigUint64(0, true));
      this.setPts(pts64);
    };
    this._rtcEngine.on('audio-metadata', this._onAudioMetadata);

    this._onStreamMessage = (_: UID, payload: Uint8Array) => {
      this.handleStreamMessage(payload);
    };
    this._rtcEngine.on('stream-message', this._onStreamMessage);
  }
  ...
  public handleMessage(
    ...
    if (isAgentMessage && this._mode === EMessageEngineMode.AUTO) {
      // Force TEXT mode to avoid PTS/Queue issues for now which cause missing messages
      this._mode = EMessageEngineMode.TEXT;
    }
  ...
  public cleanup() {
    ...
    if (this._rtcEngine) {
      if (this._onAudioMetadata) {
        this._rtcEngine.off('audio-metadata', this._onAudioMetadata);
        this._onAudioMetadata = null;
      }
      if (this._onStreamMessage) {
        this._rtcEngine.off('stream-message', this._onStreamMessage);
        this._onStreamMessage = null;
      }
    }
  ...
  private _mutateChatHistory() {
    // Explicitly map internal message items to the public IMessageListItem interface
    // extracting 'object' and 'action' from metadata if present
    const result: IMessageListItem[] = this.messageList.map((item) => ({
      uid: item.uid,
      turn_id: item.turn_id,
      text: item.text,
      status: item.status,
      object: (item.metadata as any)?.object,
      action: (item.metadata as any)?.action,
    }));
    ...
    this.onMessageListUpdate?.(result);
  }
}